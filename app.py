...
def perform_batch_prediction():
    if request.method == 'GET':
        ...
        if file and '.' in file.filename and file.filename.rsplit('.',1)[1].lower() in ALLOWED_EXTENSIONS:
            ...
            logging.info("CSV file recieved and uploaded")
            ...
            output = "Batch prediction completed"
            return render_template("batch.html", prediction_result = output, prediction_type='batch')
        else:
                return render_template('batch.html', error = 'File upload failed', prediction_type='batch')
